...
packages <- c("data.table", "Matrix", "xgboost", "randomForest", "caret", "plyr", "ggplot2", "lmtest", "sandwich", "stargazer", "SparseM",
              "pROC", "stringr", "dummies", "Metrics", "kernlab", "mlbench", "gbm", "caret", "rpart", "moments", "rattle", "rpart.plot", "RColorBrewer", "party", "partykit")
ipak(packages)
...
property_df <- property_df[order(property_df[,13], property_df[, 16], property_df[,2], property_df[, 39], property_df[,26], property_df[, 25], property_df[,21], property_df[, 4], property_df[, 22], property_df[, 19]),]
...
#but in  9 feature setup it does have  has duplicate flag, value is 1
...
#if in 9 feature it got a flag, it is truly duplicate
...
ggplot(data = property_df[property_df$sqm < 180,], aes(x = ln_sqm, y = ln_psqm)) + geom_smooth()
dev.off()
